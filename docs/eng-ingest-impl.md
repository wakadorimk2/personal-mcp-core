...
- [ ] `events.jsonl` 互換経路の移行条件が明文化されている
  ...
  - 再生成条件: `events.db` が正本であり、欠損時は `events.db` から再生成する（`docs/data-directory.md` の runtime storage steady state）
  - 復旧手順維持条件: `events.db` 欠損時は `events.jsonl` から再生成できる手順を維持する（`docs/data-directory.md` の runtime storage steady state）
...
| storage 単一化との整合確認 | `docs/data-directory.md`、#190/#191 |
