...
| `save_type` | string | `instant` / `submit` | 即保存と明示保存の分離 |
| `edited_before_submit` | bool | `true` / `false` | 候補後編集の有無判定 |

推奨（任意）:

- `candidate_selected`（bool）
- `candidate_source`（`recent` / `today_frequent` / `7d_frequent` / `fixed`）

---

## 6. 移行方針（実装は follow-up）

- 既存ログに上記メタデータが無い期間は、分析時に `unknown` として扱う。
- reader tolerance で欠損を許容し、過去データの後方互換レイヤは追加しない。
- 本契約を反映する実装は follow-up Issue で分離する。
...
- 比較混同を防ぐ最小メタデータは `mode` / `save_type` / `edited_before_submit` とする。
