...
- webhook secret は process env の `DISCORD_WEBHOOK_URL` を優先し、未設定時のみ `~/.config/secrets/discord_webhook.env` fallback を使う
...
- fallback file の想定内容は `export DISCORD_WEBHOOK_URL="https://discord.com/api/webhooks/..."`
- 一時的な検証や切り替えでは process env の `DISCORD_WEBHOOK_URL` が fallback file より優先される
...
- fallback behavior:
  - process env に `DISCORD_WEBHOOK_URL` があればそれを使い、fallback file は読まない
  - process env に `DISCORD_WEBHOOK_URL` がなければ `~/.config/secrets/discord_webhook.env` を読む
