...
### Optional `data` field: `observation_model`

Issue #332 では、`data.observation_model` を optional field として導入する。
これは telemetry 専用 field ではなく、必要なら全 event で持てる observation metadata である。

現時点で repo が新規書き込みに使う値:

| value | meaning |
|---|---|
| `"current"` | この record が current observation model の writer から出力されたことを表す |

運用ルール:

- #332 で実際に付与するのは新規 UI telemetry writer のみ
- #332 は `raw_count` / `shipped_density` の意味を変更しない
- 既存 historical record への migration は行わない
- historical fallback は issue / spec 側で明示された boundary date に従って扱い、`source` や legacy shape から暗黙推定しない
- unknown `data.observation_model` 値は v1 reader が無視して読み進めてよい
