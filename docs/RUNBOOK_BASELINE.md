...
|---|---|---|---|
| 役割 | no-side-effect 担当 | side-effect 担当 | TBD（副作用あり運用にする場合は side-effect 扱い） |
| 認証・環境 | Claude UI / API key | API key / env / `gh auth` | GitHub auth / runtime 実装に応じて確定 |
| コマンド実行 | 禁止 | 許可（検証範囲内） | TBD |
| Git 操作 | 禁止 | 許可（最小範囲） | TBD |
| PR 作成 | 禁止 | `gh pr create` | `gh pr create` を使う場合は `Linked Issue` を必須にする |
| Safety Check | 不要（副作用なし） | `git status` + guard スクリプト | PR 作成や push を許可する場合は Codex CLI と同等の safety check を要求する |
| 出力形式 | unified diff + コマンド候補 | PR Body Template | PR Body Template（`Linked Issue` を必須） |
| 停止条件 | Issue 外変更・不確実性 | 3 回収束しない・最小修正超過 | TBD |

> Copilot CLI の詳細運用は引き続き別 runbook で確定する。
> ただし、PR 作成を許可する場合でも `Linked Issue` を省略してよい runtime にはしない。
> title 末尾の `(#123)` や本文中の単なる `#123` 記載だけでは linked issue とみなさない。
...
- [ ] PR 作成を許可する runtime では、`Linked Issue` の書式と close / non-close の扱いを runbook に明記する
...
現時点では詳細運用が未確定のため、具体 runbook は別 Issue で確定する。
ただし最小要件として、Copilot CLI に PR 作成を許可する場合は次を満たす。

- Codex CLI と同じ `PR Body Template` を使う
- `## Linked Issue` を PR 本文に必須化する
- `Closes/Fixes/Resolves #<issue-number>` を close 用、`Refs #<issue-number>` を non-close 用として使い分ける
- title 末尾の `(#123)` や本文中の単なる `#123` 記載だけで完了扱いにしない
